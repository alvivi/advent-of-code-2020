...
ingredientsWithAllergens :: Map String (Set String) -> [[(String, String)]]
ingredientsWithAllergens allergenSets | Map.size allergenSets <= 0 = []
ingredientsWithAllergens allergenSets = ingredients : ingredientsWithAllergens allergenSets'
  where
...
main = do
  ...
  print $ map snd $ List.sort $ concat $ ingredientsWithAllergens $ parse contents
